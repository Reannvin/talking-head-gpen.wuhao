...
parser.add_argument('--x1',  type=int, default=0,  help='x1');
parser.add_argument('--x2',  type=int, default=0,  help='x2');
parser.add_argument('--y1',  type=int, default=0,  help='y1');
parser.add_argument('--y2',  type=int, default=0,  help='y2');
...
def inference_video(opt):
  ...
  frames_path = os.path.join(opt.frames_dir, opt.reference, '*.jpg')
  flist = [f for f in glob.glob(frames_path) if os.path.isfile(f)]
...
def scene_detect(opt):
  ...
  video_manager.start()
  # when raise type error, go the python library ./scenedetect/detectors/content_detector.py, change type of curr_hsv and last_hsv to list.
...
def get_audio_path(video_path):
    ...
    audio_dir = os.path.join(base_dir, "audio", os.path.basename(base_dir))
    audio_file = base_name.replace(".mp4", ".wav")
...
if has_audio_track(opt.videofile):
  ...
  output = subprocess.call(command, shell=True, stdout=None)
    ...
    source_audio_path = get_audio_path(opt.videofile)
    command = f"ffmpeg -y -i {source_audio_path} -ac 1 -vn -acodec pcm_s16le -ar 16000 {os.path.join(opt.avi_dir,opt.reference,'audio.wav')}"
    output = subprocess.call(command, shell=True, stdout=None)
...
# # output = subprocess.call(command, shell=True, stdout=None)
# # crop video
# curr_input_path = os.path.join(opt.avi_dir,opt.reference,'video.avi')
# curr_output_path = os.path.join(opt.crop_dir,opt.reference,'00000.avi')
# print("curr_output_path:", curr_output_path)
# cmd = f"ffmpeg -i {curr_input_path} -vf \"crop={opt.x2-opt.x1}:{opt.y2-opt.y1}:{opt.x1}:{opt.y1}\" -c:v libx264 -preset fast -crf 22 -c:a copy {curr_output_path}"
# process1 = subprocess.Popen(cmd, shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
# stdout, stderr = process1.communicate()
# if process1.returncode != 0:
#   print(f"crop video 出错: {stderr.decode('utf-8')}")
