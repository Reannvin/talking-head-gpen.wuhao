...
def get_videos_list(root_dir):
    ...
    for person_id in sorted(os.listdir(root_dir)):
        ...
        video_path = os.path.join(root_dir, person_id)
        videos_list.append((video_path, person_id))
    return videos_list
...
def audio_process(audio_path, model_path, fps, syncnet_T):
    ...
    if os.path.exists(audio_path):
        ...
        save_path = os.path.join(os.path.dirname(audio_path), "whisper.npy")
        torch.save(whisper_chunks, save_path)

def main(root_dir, model_path, fps, syncnet_T):
    videos_list = get_videos_list(root_dir)
    audio_paths = [(os.path.join(video_path, 'audio.wav'), model_path, fps) for video_path, _, in videos_list]
...
if __name__ == "__main__":
    ...
    parser.add_argument('--model_path', type=str, default='./models/tiny.pt', help='Path to Whisper model')
