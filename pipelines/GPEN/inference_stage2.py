...
print('Using {} for inference stage 2.'.format(device))
...
def load_model(args):
    ...
    args.latent = 512
    args.n_mlp = 8
    generator = FullGenerator(args.size, args.latent, args.n_mlp, channel_multiplier=args.channel_multiplier, narrow=args.narrow, device=device, stage=2)
...
def main():
    ...
    for i, (img_batch, audio_batch, frames, coords, masks) in enumerate(tqdm(gen, 
        ...
        with torch.no_grad():
            pred_image, _ = model(inputs=img_batch, audio=audio_batch)
