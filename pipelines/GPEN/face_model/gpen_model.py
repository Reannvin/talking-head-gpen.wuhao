...
class FullGenerator(nn.Module):
    def __init__(
        ...
        self.audio_encoder = nn.Sequential(nn.Conv2d(in_channels=1, out_channels=64, kernel_size=(3, 3),stride=(2,2),padding=(1, 1)),
                                            nn.ReLU(),
    ...
    def forward(self,
        ...
        noise = []
        if inputs.dim() >4:
            inputs = self.tensor5to4(inputs)
            audio = self.tensor5to4_audio(audio)
        for i in range(self.log_size-1):
            ...
            if self.stage == 2:
                ...
                noise.append(torch.randn_like(inputs))   
        if self.stage == 2 and audio.size():
            outs = self.audio_encoder(audio)  
