...
from dataset import MultiResolutionDataset, ImageVideoCombineDataset
...
def save_sample_image(real_img, fake_img,exp_name,global_step,ref_num=1):
   ...
   # sample_num=3
    ...
    rows=2
    cols=4
    real_img_save=np.zeros((256*rows,256*cols,3),dtype=np.uint8)
    fake_img_save=np.zeros((256*rows,256*cols,3),dtype=np.uint8)

    for i in range(rows):
       for j in range(cols):
           real_img_save[i*256:(i+1)*256,j*256:(j+1)*256,:]=real_img[i*cols+j]
           fake_img_save[i*256:(i+1)*256,j*256:(j+1)*256,:]=fake_img[i*cols+j]
    fake_real_concat=np.concatenate([real_img_save,fake_img_save],axis=0)
    cv2.imwrite(f"experiments/{exp_name}/sample/concat_{global_step}.png",fake_real_concat)
...
def train(args, loader, generator, discriminator, g_optim, d_optim, g_ema, device):
    ...
    for idx in pbar:
        ...
        mask,real_img = next(loader)
        real_img = real_img.to(device)
        mask = mask.to(device)
        
        mask_img=real_img.clone()
        mask_img[mask==255]=0
        ...
        # mask_img=real_img.clone()
        # mask_img[:,:,mask_img.size(2)//2:,:]=0
        fake_img = generator(mask_img,noise)
        ...
        # print("real_img_aug shape:",real_img_aug.shape)
        ...
        l1_loss=F.l1_loss(fake_img[mask==0],real_img[mask==0])
        g_loss+=10*l1_loss
        loss_dict["g"] = g_loss
        ...
        #     fake_img, latents = generator(real_img,noise, return_latents=True)
        ...
        if get_rank() == 0:
            pbar.set_description(
                (
                    ...
                    f"l1: {l1_loss:.4f}; "
                    f"fake: {fake_score_val:.4f}; real: {real_score_val:.4f}; "
                    #"r1: {r1_val:.4f}; "
            ...
            if i % 100 == 0:
                with torch.no_grad():
                    ...
                    save_sample_image(mask_img, fake_img2,args.exp_name,i)
                    # utils.save_image(
                    #     sample[:, [2, 1, 0], :, :],
...
if __name__ == "__main__":
    ...
    if args.arch == 'stylegan2':
        ...
        from models import InpaintGenerator as Generator
    ...
    dataset=ImageVideoCombineDataset(args.path, transform, args.size)
