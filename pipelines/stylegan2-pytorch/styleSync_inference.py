...
from models import FullGenerator,EmbeddingGenerator
from warp_mouth import warp_mouth
...
parser.add_argument('--stylegan', help='using stylegan as arch', action='store_true')
...
parser.add_argument('--warp_mouth',action='store_true', help='mouth warp')
parser.add_argument('--warp_mouth_id',default=0, type=int, help='mouth warp template id')
...
def warp_mouth_landmark(frames,template_idx=0):
    ...
    template_results = inference_topdown(model, template_frame)
    template_results = merge_data_samples(template_results)
    template_keypoints = template_results.pred_instances.keypoints
    template_landmark= template_keypoints[0][23:91]
    template_landmark = template_landmark.astype(np.int32)
   # template_mouth_landmark=template_landmark[48:60]
    coords_list = []
    landmarks = []
    average_range_minus = []
    average_range_plus = []
    for i in tqdm(range(len(frames))):
        if i==template_idx:
            continue
        results = inference_topdown(model, frames[i])
        results = merge_data_samples(results)
        keypoints = results.pred_instances.keypoints
        face_landmark= keypoints[0][23:91]
        face_landmark = face_landmark.astype(np.int32)
        
        frames[i]=warp_mouth(template_frame,frames[i],template_landmark,face_landmark )
...
def datagen(frames, audios):
    ...
    if args.warp_mouth:
        warp_mouth_landmark( frames,template_idx=args.warp_mouth_id)
    ...
    if args.arc_face:
        ...
        if not args.static:
            if args.crop_landmark :
                landmark_dir=os.path.join(os.path.dirname(args.face),'landmark_coords')
                if not os.path.exists(landmark_dir):
                    os.makedirs(landmark_dir)
                coords_path=os.path.join(landmark_dir,os.path.basename(args.face).split('.')[0]+'.npy')
            else:
                s3fd_dir=os.path.join(os.path.dirname(args.face),'s3fd_coords')
                if not os.path.exists(s3fd_dir):
                    os.makedirs(s3fd_dir)
                coords_path=os.path.join(s3fd_dir,os.path.basename(args.face).split('.')[0]+'.npy')
            if args.crop_landmark and (args.lmk_ratio!=0 or args.lmk_pad!=0):
    ...
    if args.double_detect:
        ...
        for i in range(len(frames)):
            ...
            crop_pad_frame.append(frames[i][y1-col_pad: y2+col_pad, x1-row_pad:x2+row_pad])
        if not args.static:
            face_det_results = face_detect(crop_pad_frame) # BGR2RGB for CNN face detection
        else:
            face_det_results = face_detect([crop_pad_frame[0]])
    ref_idx = random.sample(range(len(frames)-1), args.ref_num-1)
    for i, a in enumerate(audios):
        ...
        if args.ref_num>1:
            # if len(frames)<args.ref_num:
            #     ref_idx=random.sample(range(len(frames)-1),len(frames)-1)
            #     ref_idx=ref_idx+ref_idx[::-1]
            #     ref_idx=ref_idx[:args.ref_num]
          ...
          #  print(ref_idx)
            #ref_idx = random.sample(range(len(frames)-1), args.ref_num-1)
            ref_idx=0,7
            refs = [face_det_results[k].copy() for k in ref_idx]
            for ref,_ in refs:
                ref=cv2.resize(ref,(args.image_size,args.image_size))
                face = np.concatenate((ref,face), axis=2)
...
def load_model(path):
    generator = FullGenerator(
        args.image_size, 512, 8, channel_multiplier=2,ref_num=args.ref_num
    ).to(device)
...
def main():
    ...
    for i, (img_batch, audio_batch, frames, coords, metric) in enumerate(tqdm(gen, 
        ...
        if args.save_sample:
           ...
           # if args.ref_num>1:
            ref_img_list=[]
            for j in range(args.ref_num):
                ref_img_list.append(ref_img[:,:,:,3*j:3*(j+1)])
            ref_concat_img=np.concatenate(ref_img_list,axis=2)
            concat_batch = np.concatenate((ref_concat_img, mask_img,pred), axis=2)
