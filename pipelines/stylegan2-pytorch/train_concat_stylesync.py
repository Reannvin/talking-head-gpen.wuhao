...
def save_sample_image(real_img, fake_img, con_img,exp_name,global_step,ref_num=1):
    ...
    sample_num=min(real_img.shape[0],8)
...
def train(args, loader, generator, discriminator,g_optim, d_optim, g_ema, unet,device):
    unet=unet.eval()
    L1_loss=nn.L1Loss()
    ...
    for idx in pbar:
        ...
        #     con_image=tensor5to4(con_image)
        #     con_audio=tensor5to4_audio( con_audio)
        with torch.no_grad():
            input_size=real_img.size()
            if len(input_size)>4:
                con_image_tmp=tensor5to4(con_image)
            else:
                con_image_tmp=con_image
            con_latent=encode_face(vae,con_image_tmp)
            real_latent=encode_face(vae,real_img)
            con_audio2=tensor5to3_audio(con_audio)
            zero_timestep = torch.zeros([]).to(device)
            fake_latent = unet(con_latent, timestep=zero_timestep, encoder_hidden_states=con_audio2).sample    
            fake_img_mids = decode_face(vae,fake_latent)
            fake_img_mids=tensor4to5(fake_img_mids,input_size[0],input_size[2]).clamp(0, 1)
            if args.drop_pose_prob>0:
                drop_prob = random.random()
                if drop_prob < args.drop_pose_prob:
                    fake_img_mids[:,:,:,fake_img_mids.size(3)//2:,:]=torch.randn(fake_img_mids[:,:,:,fake_img_mids.size(3)//2:,:].size()).to(device)
            con_image2=torch.concat([fake_img_mids,con_image[:,3:]],dim=1)
        if args.gan_loss_wt>0 and idx<1000:
            requires_grad(generator, False)
            requires_grad(discriminator, True)
            
            fake_img=generator(con_image2,con_audio)
            if args.augment:
       ...
       # l1_loss = L1_loss(fake_img[:,:,fake_img.size(2)//2:,:], real_img[:,:,real_img.size(2)//2:,:])
        ...
        if get_rank() == 0:
            ...
            if i % args.sample_images == 0:
                ...
                save_sample_image(real_img, fake_img,con_image2, args.exp_name,i,args.ref_num)
...
if __name__ == "__main__":
   ...
   # parser.add_argument("path", type=str, help="path to the lmdb dataset")
    ...
    parser.add_argument('--latent_loss_wt', type=float, default=0, help='latent loss weight')
    ...
    parser.add_argument('--drop_pose_prob', type=float, default=0, help='drop ref prob')
    parser.add_argument('--disc_ckpt', type=str, help='Path to the disc checkpoint to load the model from')
    parser.add_argument('--unet_ckpt', type=str, help='Path to the gen checkpoint to load the model from')
    ...
    if args.unet_ckpt is not None:
        print("load unet model from:", args.unet_ckpt)

        if args.unet_ckpt.endswith('.safetensors'):
            from safetensors.torch import load_file
            new_s = load_file(args.unet_ckpt)
        else:
            s = torch.load(args.unet_ckpt, map_location=device)
    ...
    train(args, loader, generator, discriminator, g_optim, d_optim, g_ema, unet,device)
