...
from models import Generator,EmbeddingGenerator
from train_con import make_noise
...
def get_bbox_by_landmark(model, fa, frames, upperbondrange =0,lmk_ratio=0):
    ...
    for fb in batches:
        ...
        if args.warp_mouth:
            if first_frame==np.asarray(fb)[0]:
                warp_mouth_landmark
                warp_mouth(first_frame,np.asarray(fb)[0],face_land_mark,face_land_mark)
            mouth_landmark = face_land_mark[48:60]
...
def datagen(frames, audios):
    ...
    if len(img_batch) > 0:
        ...
        img_batch = (np.concatenate((img_masked, img_batch), axis=3) / 255)*2-1.
...
def load_model(path):
    generator = Generator(
        args.image_size, 512, 8, channel_multiplier=2
    ).to(device)
    ...
    emb_g.load_state_dict(new_s)
    return generator.eval(), emb_g.eval()
...
def main():
    ...
    for i, (img_batch, audio_batch, frames, coords, metric) in enumerate(tqdm(gen, 
        ...
        with torch.no_grad():
            tf=emb_g(audio_batch,img_batch)
            noise= torch.randn(img_batch.size(0), 512, device=device)
            if args.no_noise:
                noise=tf
            elif not args.no_con:
                noise = [n+tf for n in noise]
            pred_image ,_= g(noise)
        ...
        pred = ((pred_image+1)/2).clamp(0, 1).cpu().numpy().transpose(0, 2, 3, 1) * 255.
        if args.save_sample:
