...
class PerceptualLoss(nn.Module):
    ...
    def forward(self, inp_source, target_source, mask=None,use_style_loss=False,weight_style_to_perceptual=0.,warp=False):
        if len(inp_source.size())>4:
            b, c, t, h, w = inp_source.size()
            inp_source = inp_source.permute(0, 2, 1, 3, 4).reshape(-1, c, h, w)
        if len(target_source.size())>4:
            b, c, t, h, w = target_source.size()
            target_source = target_source.permute(0, 2, 1, 3, 4).reshape(-1, c, h, w)
