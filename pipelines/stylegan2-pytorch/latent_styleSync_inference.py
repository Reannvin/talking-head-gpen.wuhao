...
parser.add_argument('--ref_num', type=int, help='number of reference images', default=1)
parser.add_argument('--gs_blur', action='store_true', help='gs blur when mask lower half')
parser.add_argument('--arch', type=str, help='model arch', default='stylegan')
...
def load_model(path):
    if args.arch=='unet':
        ...
        generator = LatentGenerator(
            args.image_size//8, 512, 8, channel_multiplier=2
        ).to(device)
    ...
    for i, (img_batch, audio_batch, frames, coords, metric) in enumerate(tqdm(gen, 
        ...
        with torch.no_grad():
            ...
            if args.arch=='unet':
                ...
                # print('image latent size:',image_latent.size())
                # print('audio batch size:',audio_batch.size())
                pred_latent = g(image_latent, timestep=zero_timestep, encoder_hidden_states=audio_batch).sample
            else:
                pred_latent = g(image_latent, audio_batch)
