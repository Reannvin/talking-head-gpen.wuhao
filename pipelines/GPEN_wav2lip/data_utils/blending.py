...
# upper_boundarr_ratio was 0.4 before
def get_image(image,face,face_box,upper_boundary_ratio = 0.4,expand=1.2,debug=True, enable_erode=False, enable_dilate=False):
    ...
    modified_mask_image_2 = Image.new('L', ori_shape, 0)
    modified_mask_image_2.paste(mask_image.crop((0, top_boundary, width, height)), (0, top_boundary))
    ...
    res_face = body.crop(crop_box)
    res_face.save("res_face.jpg")

    body = np.array(body)
    return body[:,:,::-1]

def get_image_prepare_material(image,face_box,upper_boundary_ratio = 0.4,expand=1.2):
    body = Image.fromarray(image[:,:,::-1])
