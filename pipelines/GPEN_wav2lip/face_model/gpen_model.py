...
class Generator(nn.Module):
    ...
    def forward(
        ...
        if noise is None:
            ...
            for i in range(self.log_size + 1, 2, -1):  
                size = 2 ** (i-1)
...
class AudioEncoderStylesync(nn.Module):
...
class FullGenerator(nn.Module):
    def __init__(
        ...
        if pretrained_generator is not None:
            self.generator.load_state_dict(pretrained_generator)
        
        self.is_concat = concat_condition
        ...
        if sync_audio_encoder:
            self.audioencoder = AudioEncoderStylesync()
        else:
            self.audioencoder = AudioEncoder()
        if concat_condition:
            self.facial_encoder = nn.Sequential(
                torch.nn.Linear(512*16, 256),
                torch.nn.ReLU(),
            )
    ...
    def forward(self,
        ...
        facial_feature_noise = []
        cond_image = cond_image_inputs
        for i in range(self.log_size-1):
            ecd = getattr(self, self.names[i])
            cond_image = ecd(cond_image)
            facial_feature_noise.append(cond_image)

        facial_feature_noise = list(itertools.chain.from_iterable(itertools.repeat(x, 2) for x in facial_feature_noise))[::-1]
        cond_audio_outputs = self.audioencoder(cond_audio_inputs)
        if self.is_concat:
            cond_image = cond_image.reshape(cond_image.shape[0],-1)
            cond_image = self.facial_encoder(cond_image)
            input = self.input_encoder(torch.cat([cond_audio_outputs, cond_image], dim=1))
            outs = self.generator([input], return_latents, inject_index, truncation, truncation_latent, input_is_latent, noise=facial_feature_noise[1:])
        else:
            outs = self.generator([cond_audio_outputs], return_latents, inject_index, truncation, truncation_latent, input_is_latent, noise=facial_feature_noise[1:])
        return outs
