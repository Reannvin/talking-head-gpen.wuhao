...
from face_model.gpen_model import Generator, Discriminator
...
def train(args, loader, generator, discriminator, g_optim, d_optim, g_ema, device):
    ...
    for idx in pbar:
        ...
        gen_z = mixing_noise(args.batch, args.latent, 0, device)
            
        fake_img, _ = generator(gen_z, noise=None)
        ...
        if g_regularize:
            ...
            fake_img, latents = generator(gen_z, noise=None, return_latents=True)
            ...
            if i % args.save_sample_freq == 0:
                ...
                with torch.no_grad():
                    ...
                    for idx in range(sample.shape[0]):
                        save_image(sample[idx], os.path.join(snapshot_folder, f'sample_{idx}.png'))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--n_sample', type=int, default=64, help="number of samples generated during training")
    ...
    generator = Generator(
        args.size, args.latent, args.n_mlp, channel_multiplier=args.channel_multiplier, narrow=args.narrow, isconcat=True, device=device
    ).to(device)
