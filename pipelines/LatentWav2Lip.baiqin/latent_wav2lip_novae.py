        ...
        while 1:
          #  print('get item')
           ...
           # print(img_name)
            ...
            # window_upper_half = window_full_image.clone()
            # window_upper_half[:,:,window_upper_half.size(2)//2:,:] = 0
...
def decode_latent(face_sequences,device='cuda:0',decode_num=4):
    face_sequences=face_sequences[:decode_num]

    from diffusers import AutoencoderKL
    vae=AutoencoderKL.from_pretrained('models/SD_MSE',use_safetensors=True)
    vae.eval()
    vae.to(device)
    ...
    print("face_sequences shape:",face_sequences.shape)
    for i in tqdm(range(0, face_sequences.shape[0], batch_size)):
        batch_input_tensor = face_sequences[i:i + batch_size].to(device)
        with torch.no_grad():
            # 编码
        #  print("batch_input_tensor shape:",batch_input_tensor.shape)
            rec = vae.decode(batch_input_tensor).sample


            output_tensor = rec if output_tensor is None else torch.cat([output_tensor, rec],dim=0)
    if input_dim_size > 4:
            output_tensor = torch.split(output_tensor, B, dim=0) # [(B, C, H, W)]
           # x=self.vae.decode(x).sample
            output_tensor = torch.stack(output_tensor, dim=1) # (B, T, C, H, W)


    outputs = (((output_tensor+1)/2.0).clamp(0, 1)*255).byte().cpu().numpy().transpose(0, 1, 3, 4, 2).astype(np.uint8)
...
def save_sample_images(x, g, gt, global_step, checkpoint_dir):
    ...
    inps=x[:,:4,:,:,:]
    print(refs.shape)
    refs=decode_latent(refs)
    inps=decode_latent(inps)
    g=decode_latent(g)
    gt=decode_latent(gt)
   ...
   # refs, inps = x[..., 3:], x[..., :3]
    ...
    if not os.path.exists(sample_image_dir): os.mkdir(sample_image_dir)
    folder = join(sample_image_dir, "samples_step{:09d}".format(global_step))
...
class LatentWav2Lip(pl.LightningModule):
    ...
    def validation_step(self, batch, batch_idx):
        ...
        if batch_idx==0 and self.device.index==0 and self.hparams.sample_images:
            save_sample_images(x, g, gt, self.global_step, self.trainer.checkpoint_callback.dirpath)
        return recon_loss

    def on_validation_epoch_end(self) -> None:
        val_sync_loss = self.trainer.logged_metrics['val_sync_loss']
        if val_sync_loss < .75:
            print(f"Syncnet loss {val_sync_loss} is less than 0.75, setting syncnet_wt to 0.03")
            self.hparams.syncnet_wt = 0.03 # switch on syncnet loss
    ...
    def val_dataloader(self):
        ...
        print("val dataset length:",len(test_dataset))
...
if __name__ == "__main__":
    ...
    parser.add_argument('--sample_images', action='store_true', help=' save Sample image to decode')
    ...
    checkpoint_callback = pl.callbacks.ModelCheckpoint(
        filename='wav2lip-{epoch}-{step}-{train_loss:.2f}-{val_loss:.2f}-{val_sync_loss:.2f}',
        save_top_k=3,
        verbose=True,
        monitor='val_loss',
        mode='min'
