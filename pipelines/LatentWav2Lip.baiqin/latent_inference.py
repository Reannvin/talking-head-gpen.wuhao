...
parser.add_argument('--vae_path', type=str, help='Path to the VAE model', default='models/SD_MSE')
...
parser.add_argument('--noparse', default=False, action='store_true',
					help='Prevent face segmentation')
...
def to_latent(vae, img):
	...
	# # masked_image = resized_image[:, :3]
	# # masked_image[:, :, masked_image.size(2) // 2:] = 0
	
	# # # rescale img to [-1, 1]
	# # upper_half_image = 2 * masked_image - 1
	# # reference_image = 2 *resized_image[:, 3:]-1
	...
	# #在image上mask掉下半脸，然后encode得到latent
	# upper_half_image = masked_image[:, :, :masked_image.size(2) // 2]
	
	# lower_half_zeros = torch.zeros_like(upper_half_image)
	# upper_half_image = torch.cat([upper_half_image, lower_half_zeros], dim=2)

	# reference_image = rescaled_image[:, 3:]
    
	# with torch.no_grad():
	# 	upper_half_latent = vae.encode(upper_half_image).latent_dist.sample()
	...
	# input_latent_sticth = upper_half_latent

	# #下半脸用gt到latent后再mask
	# upper_half_image=masked_image.clone()	
	# reference_image = rescaled_image[:, 3:]
	# with torch.no_grad():
	# 	upper_half_latent = vae.encode(upper_half_image).latent_dist.sample()
	# 	upper_half_latent[:,:,upper_half_latent.size(2)//2:,:]=0
	# 	reference_latent = vae.encode(reference_image).latent_dist.sample()
	# input_latent_sticth = upper_half_latent

	latent_path = '/data/wangbaiqin/dataset/HDTF_latent_halfblack/RD_Radio2/0000/0.pt'
	latent = torch.load(latent_path)
	#print(img.shape)
	input_latent_sticth = latent['upper_half'].unsqueeze(0).to(device)
	reference_latent = latent['full_image'].unsqueeze(0).to(device)
	return torch.cat([input_latent_sticth, reference_latent], dim=1)
...
def main():
	...
	for i, (img_batch, mel_batch, frames, coords) in enumerate(tqdm(gen, 
		...
		with torch.no_grad():
			pred_latent = model(mel_batch, image_latent)
