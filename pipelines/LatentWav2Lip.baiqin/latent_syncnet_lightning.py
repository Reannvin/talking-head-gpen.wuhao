...
class AudioVisualDataset(Dataset):
    def __init__(self, split, need_negative_sample, multiply):
...
class LatentSyncNet(pl.LightningModule):
    ...
    def train_dataloader(self):
        if self.hparams.clip_loss:
            ...
            train_dataset = AudioVisualDataset('main', need_negative_sample=False, multiply=self.hparams.multiply) 
        else:
            # train with cosine loss, negative sample wanted
            train_dataset = AudioVisualDataset('main', need_negative_sample=True, multiply=self.hparams.multiply)
    ...
    def val_dataloader(self):
        test_dataset = AudioVisualDataset('val', need_negative_sample=True, multiply=1) # val with cosine loss, negative sample wanted
