...
def iterate_random_labels(opts, batch_size, device=torch.device('cuda')):
    training_set = dnnlib.util.construct_class_by_name(**opts.dataset_kwargs)
    train_dataset = AudioVisualDataset(data_root = '/mnt/hdtf_btm_move/', 
                                    dataset_name = 'hdtf', 
                                    syncnet_T = 1,
                                    split = 'main')
    ...
    while True:
        ...
        noise_window = torch.randn(batch_size, 3, 1, 256, 256, dtype=torch.float32, device=device)
        ...
        c_audio = c_audio.reshape(-1, 50, 384)
        yield c_image, c_audio
...
def compute_feature_stats_for_generator(opts, detector_url, detector_kwargs, rel_lo=0, rel_hi=1, batch_size=64, batch_gen=None, **stats_kwargs):
    ...
    while not stats.is_full():
        ...
        for _i in range(batch_size // batch_gen):
            ...
            c_image, c_audio = next(c_iter)
            img = G(z=z, c_i=c_image, c_a=c_audio, **opts.G_kwargs)
