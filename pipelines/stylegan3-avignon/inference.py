...
def generate_images(pkl, seeds, truncation_psi, noise_mode, outdir, translate, rotate):
    print('Load model from StyleGAN3 Arch.')
...
def main():
    ...
    for i, (img_batch, audio_batch, frames, coords, masks) in enumerate(tqdm(gen, 
        ...
        # -------------------------------------------------------
        noise = torch.randn(img_batch.shape[0], 3, 256, 256, device=device)
        img_batch = torch.cat([img_batch, noise], dim=1) # 16, 9, 256, 256

        img_batch = img_batch.reshape(16, 9, -1)
        img_batch = img_batch.mean(dim=-1, keepdim=True).repeat(1, 1, 256)

        audio_batch = audio_batch.squeeze(1)
        # img_batch = torch.nn.functional.interpolate(img_batch, size=(512, 512), mode='bilinear', align_corners=False)
        # -------------------------------------------------------

        img_batch = img_batch.split(4)
        audio_batch = audio_batch.split(4)

        # pred_image = from_latent(vae, g_latent) # Tensor
        # network_pkl = '/data/wuhao/code/stylegan3-avignon/training-runs/00028-stylegan3-t--gpus1-batch16-gamma8.2/network-snapshot-000200.pkl'
        
        network_pkl = args.checkpoint_path
        ...
        pred_image = torch.cat([G_ema(z_image = img, z_audio = audio,c = label,noise_mode='const') for img, audio in zip(img_batch, audio_batch)]).cpu()
        # pred_image = G(z, label, truncation_psi= 1, noise_mode='const')
