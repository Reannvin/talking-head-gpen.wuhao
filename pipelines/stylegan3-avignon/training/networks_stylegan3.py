...
class VAE(torch.nn.Module):# Input shape (16, 9, 256, 256) -> [16, 4, 32, 32]
    def __init__(self, pretrained_vae_model="stabilityai/sd-vae-ft-mse"):
        super(VAE, self).__init__()
        
        self.vae = AutoencoderKL.from_pretrained(pretrained_vae_model, local_files_only=True)
        self.vae.eval()
        
        for param in self.vae.parameters():
            param.requires_grad = False

        self.channel_mapper = torch.nn.Conv2d(9, 3, kernel_size=1, stride=1, padding=0)
...
class Generator(torch.nn.Module):
    ...
    def forward(self, z, c_i, c_a, truncation_psi=1, truncation_cutoff=None, update_emas=False, **synthesis_kwargs):
        # print(c_i.shape) # ([80, 9, 256, 256])
        # print(c_a.shape) # ([80, 50, 384])
