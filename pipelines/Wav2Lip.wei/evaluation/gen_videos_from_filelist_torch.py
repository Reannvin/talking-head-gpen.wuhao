...
parser.add_argument('--results_dir', type=str, help='Folder to save all results into', 
                                    required=True)
parser.add_argument('--real_root', type=str, help='Folder to save all results into', 
                                    required=True)
parser.add_argument('--syn_root', type=str, help='Folder to save all results into', 
                                    required=True)
...
def datagen(frames, face_det_results, mels):
    ...
    for i, m in enumerate(mels):
        ...
        if len(img_batch) >= args.wav2lip_batch_size:
            ...
            mel_batch = torch.stack(mel_batch, dim=0)
...
def main():
    audio_transforms = audio.get_audio_transforms()
    assert args.data_root is not None
    data_root = args.data_root

    if not os.path.isdir(args.results_dir): os.makedirs(args.results_dir)

    start, end = 2000 * chunk_ind, 2000 * (chunk_ind + 1)
    with open(args.filelist, 'r') as filelist:
        lines = filelist.readlines()[start:end]
    print(f'start:{start} end:{end} len:{len(lines)}')
    for idx, line in enumerate(tqdm(lines)):
        ...
        command = 'ffmpeg -loglevel panic -y -i {} -strict -2 {}'.format(audio_src, f'../temp/temp_{chunk_ind}.wav')
        subprocess.call(command, shell=True)
        temp_audio = f'../temp/temp_{chunk_ind}.wav'

        # wav = audio.load_wav(temp_audio, 16000)
        # mel = audio.melspectrogram(wav)
        wav, sample_rate = torchaudio.load(temp_audio, normalize=True)
        if sample_rate != 16000:
            print('resample')
            wav = torchaudio.functional.resample(wav, sample_rate,  16000)
        mel = audio_transforms(wav)

        if torch.isnan(mel.reshape(-1)).sum() > 0:
            continue
        ...
        while 1:
            ...
            if start_idx + mel_step_size > len(mel[0, 0]):
                break
        ...
        for i, (img_batch, mel_batch, frames, coords) in enumerate(gen):
            if i == 0:
                ...
                out = cv2.VideoWriter(f'../temp/result_{chunk_ind}.avi',
            ...
            for pl, f, c in zip(pred, frames, coords):
                #*****************
                save_real_path = Path(args.real_root) / f'{start+idx}_{frames_cnt}.jpg'
                save_syn_path = Path(args.syn_root) / f'{start+idx}_{frames_cnt}.jpg'
                frames_cnt += 1
                save_real_path.parent.mkdir(exist_ok=True, parents=True)
                save_syn_path.parent.mkdir(exist_ok=True, parents=True)
                #----------------
                y1, y2, x1, x2 = c
                #********************
                tmp_face = cv2.resize(f[y1:y2, x1:x2], (args.img_size, args.img_size))
                cv2.imwrite(str(save_real_path), tmp_face)
                cv2.imwrite(str(save_syn_path), pl.astype(np.uint8))
                #----------------
        ...
        vid = os.path.join(args.results_dir, '{}.mp4'.format(start + idx))

        command = 'ffmpeg -loglevel panic -y -i {} -i {} -strict -2 -q:v 1 {}'.format(temp_audio, 
                                f'../temp/result_{chunk_ind}.avi', vid)
        subprocess.call(command, shell=True)
