...
def main():
	...
	for i in range(128):
		...
		if len(chunk_lines) == 0:
			continue
		print(f'start:{start} end:{end} actual len:{len(chunk_lines)}, device:{device}')
		p = multiprocessing.Process(target=main_sub, args=(args, chunk_lines, device, data_root, i))
		processes.append(p)
		p.start()
...
def main_sub(args, lines, device, data_root, sub_pro_id):
	...
	for idx, line in enumerate(tqdm(lines)):
		...
		command = 'ffmpeg -loglevel panic -y -i {} -strict -2 {}'.format(audio_src, f'../temp/temp_{sub_pro_id}.wav')
		# absolute_path = os.path.abspath(f'../temp/temp_{sub_pro_id}.wav')
		# print(f'absolute_path:', absolute_path)
		print('cmd:', command)

		subprocess.call(command, shell=True)
		temp_audio = f'../temp/temp_{sub_pro_id}.wav'
		print(f'audion extraction:{time.time() - t0}')
		t0 = time.time()

		wav = audio.load_wav(temp_audio, 16000)
		mel = audio.melspectrogram(wav)
		print(f'audion mel:{time.time() - t0}')
		t0 = time.time()
		...
		print(f'audion mel chunks:{time.time() - t0}')
		t0 = time.time()
		video_stream = cv2.VideoCapture(video)
		print(f'video open:{time.time() - t0}')
		t0 = time.time()
		...
		print(f'video read frame:{time.time() - t0}')
		t0 = time.time()
		...
		try:
			face_det_results = face_detect(full_frames.copy(), detector)
		except ValueError as e:
			continue
		print(f'face det:{time.time() - t0}')
		t0 = time.time()
		...
		for i, (img_batch, mel_batch, frames, coords) in enumerate(gen):
			if i == 0:
				...
				out = cv2.VideoWriter(f'../temp/result_{sub_pro_id}.avi',
