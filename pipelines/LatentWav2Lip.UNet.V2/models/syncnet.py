...
class SyncNet_latent(nn.Module):
    def __init__(self, syncnet_T, is_whisper, init_logit_scale: float = np.log(1 / 0.07)):
        super(SyncNet_latent, self).__init__()
        self.is_whisper = is_whisper
        self.logit_scale = nn.Parameter(torch.ones([]) * init_logit_scale)
        
        self.face_encoder = nn.Sequential(
            Conv2d(4 * syncnet_T, 128, kernel_size=(7, 7), stride=1, padding=3),

            Conv2d(128, 128, kernel_size=5, stride=(2, 2), padding=1),
            Conv2d(128, 128, kernel_size=3, stride=1, padding=1, residual=True),
            Conv2d(128, 128, kernel_size=3, stride=1, padding=1, residual=True),
        ...
        self.audio_adapter = nn.Sequential(
            ...
            # 使用1x1卷积调整通道数
            Conv2d(256, 512, kernel_size=(1, 1), stride=1, padding=0), 
        )
...
class SyncNet_latent_xl(nn.Module):
    def __init__(self, syncnet_T, is_whisper, init_logit_scale: float = np.log(1 / 0.07)):
        super(SyncNet_latent_xl, self).__init__()
        self.is_whisper = is_whisper
        self.logit_scale = nn.Parameter(torch.ones([]) * init_logit_scale)
