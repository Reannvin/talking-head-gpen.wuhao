...
class FullGenerator(nn.Module):
    def __init__(
        ...
        self.audio_adapter = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Conv2d(in_channels=512, out_channels=512, kernel_size=(3, 3), stride=(1, 6), padding=(1, 0)),
            nn.ReLU()
        )
    ...
    def forward(self,
        ...
        # split image inputs into masked and ref image
        masked, ref = torch.split(image_inputs, [3, 3], dim=1)
        ...
        ref_outs = self.final_linear(ref)
        
        # audio style    
        audio_inputs = self.audio_adapter(audio_inputs)
        audio_inputs = audio_inputs.view(audio_inputs.shape[0], -1)
        audio_outs = self.final_linear(audio_inputs)
        
        # generate
        outs = ref_outs + audio_outs
...
class Discriminator(nn.Module):
    def __init__(self, size, T=1, channel_multiplier=2, blur_kernel=[1, 3, 3, 1], narrow=1, device='cpu'):
        super().__init__()
        ...
        convs = [ConvLayer(3 * T, channels[size], 1, device=device)]
