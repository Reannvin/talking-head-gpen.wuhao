...
class LatentSyncNet(pl.LightningModule):
    ...
    def train_dataloader(self):
        if self.hparams.clip_loss:
            ...
            train_dataset = AudioVisualDataset('train', need_negative_sample=False, dataset_size=512000) 
        else:
            # train with cosine loss, negative sample wanted
            train_dataset = AudioVisualDataset('train', need_negative_sample=True, dataset_size=512000)
    ...
    def val_dataloader(self):
        test_dataset = AudioVisualDataset('val', need_negative_sample=True, dataset_size=5120) # val with cosine loss, negative sample wanted
...
if __name__ == "__main__":
    ...
    checkpoint_callback = pl.callbacks.ModelCheckpoint(
        filename='syncnet-w-{epoch}-{step}-{train_loss:.3f}-{val_loss:.3f}',
        save_top_k=3,
        verbose=True,
        monitor='val_loss',
        mode='min'
