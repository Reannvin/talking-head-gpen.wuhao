...
class LatentWav2Lip(pl.LightningModule):
    def __init__(self, hparams):
        ...
        self.unet = UNet2DConditionModel.from_pretrained(self.hparams.unet_config, local_files_only=True)        
        self.unet.eval()  # Ensure unet is always in eval mode        
        
        # 冻结 unet 的所有参数
        for param in self.unet.parameters():
            param.requires_grad = False
            
        self.controlnet = ControlNetModel.from_unet(self.unet, conditioning_channels=6)
    ...
    def reshape_face_sequences_for_unet(self, tensor): # [8, 6, 5, 768, 768] -> [40, 6, 768, 768]
        batch_size, channels, groups, height, width = tensor.shape
        reshaped_tensor = tensor.permute(0, 2, 1, 3, 4).contiguous().view(batch_size * groups, channels, height, width) 
        return reshaped_tensor
    ...
    def forward(self, audio_condition, image_condition, noisy_target, timestep):
        image_condition = self.reshape_face_sequences_for_unet(image_condition)
        ...
        if self.hparams.cfg and not self.training:
            assert False, "Classifier Free Guidance is not supported yet"
            ...
            down_block_res_samples, mid_block_res_sample = self.controlnet(noisy_target,
                timestep=timestep,
                encoder_hidden_states=audio_condition,
                controlnet_cond=image_condition,
                return_dict=False
            )
                                                                            
            g_niose = self.unet(noisy_target, 
                timestep=timestep, 
                encoder_hidden_states=audio_condition, 
