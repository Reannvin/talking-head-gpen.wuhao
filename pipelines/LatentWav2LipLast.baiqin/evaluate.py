...
def run_inference(args,video_path,audio_path,sync_path='../Evaluation/syncnet_python'):
    ...
    if args.crop_landmark:
        command=['python','unet_inference.py','--ckpt',args.ckpt,'--face',video_path,'--audio',audio_path,'--crop_landmark','--eval','--sf3d_up']#,'--gs_blur','--save_sample']
    elif args.arcface:
        command=['python','unet_inference.py','--ckpt',args.ckpt,'--face',video_path,'--audio',audio_path,'--arc_face','--eval']
    else:
        command=['python','unet_inference.py','--ckpt',args.ckpt,'--face',video_path,'--audio',audio_path,'--eval']
    if args.acc:
        command.append('--acc')
    if args.gs_blur:
...
if __name__=="__main__":
    ...
    video_list = ['data/video/example1.mp4','data/video/girl.mp4','data/video/wbq.jpg','data/video/zhuxiangyu_black.mp4']
    audio_list =['data/audio/english.wav','data/audio/liuwei_30s.mp3','data/audio/newyear.wav','data/audio/example1.wav']
