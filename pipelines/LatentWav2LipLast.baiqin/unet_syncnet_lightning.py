...
class AudioVisualDataset(Dataset):
    def __init__(self, split, need_negative_sample, dataset_name, args, dataset_size=512000):
    ...
    def __getitem__(self, idx):
        while 1:
            ...
            if args.audio_root:
                ...
                vidname = vidname.replace(args.data_root, args.audio_root)
...
if __name__ == "__main__":
    ...
    #     filename='syncnet-i-' + args.dataset_name + ('-whisper-' if args.whisper else '-wav2vec2-') + ('-T=') + str(args.syncnet_T) + '-{epoch}-{step}-{train_loss:.3f}-{val_loss:.3f}',
    #     save_top_k=3,
    #     verbose=True,
    #     monitor=monitor,
    #     mode='min'
