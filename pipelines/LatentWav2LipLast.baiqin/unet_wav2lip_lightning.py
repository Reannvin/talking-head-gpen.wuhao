...
class AudioVisualDataset(Dataset):
    def __init__(self, data_root, dataset_name, split, args, audio_root=None, dataset_size=512000,up_ratio=0,gs_blur=False):
    ...
    def __getitem__(self, idx):
        while 1:
            ...
            if self.gs_blur:
                ...
                window[:,:,window.shape[2]//2:]=np.random.normal(0, 30, (window.shape[0],window.shape[1],window.shape[2]//2,window.shape[3]))
            else:
                window[:, :, window.shape[2]//2:] = 0.
...
class LatentWav2Lip(pl.LightningModule):
    ...
    def train_dataloader(self):
        train_dataset = AudioVisualDataset(self.hparams.data_root, 
                                           ...
                                           up_ratio=self.hparams.up_ratio,
                                           gs_blur=self.hparams.gs_blur) 
        if self.hparams.ft_dataset:
            fine_tune_dataset = AudioVisualDataset(self.hparams.ft_root, 
                                                   ...
                                                   up_ratio=self.hparams.up_ratio,
                                                   gs_blur=self.hparams.gs_blur)
            train_dataset = CombinedDataset(train_dataset, fine_tune_dataset,base_ratio=0.4)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--ft_root', type=str, help='Root folder of the fine-tune dataset',default='/data/wangbaiqin/dataset/all_images')
    parser.add_argument('--ft_audio_root', type=str, help='Root folder `of the fine-tune audio dataset', default='/data/wangbaiqin/dataset/all_audios')
    ...
    parser.add_argument('--lpips_weight', type=float, default=1.0, help='Weight for LPIPS loss')
    parser.add_argument('--style_loss_wt', type=float, default=1.0, help='Weight for Style loss')
