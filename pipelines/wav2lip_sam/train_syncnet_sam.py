...
def train(device, model, train_data_loader, test_data_loader, optimizer,
    ...
    while global_epoch < nepochs:
        ...
        try:
            ...
            # Wrap the train_data_loader with tqdm for a progress bar
            ...
            for step, (x, mel, y) in enumerate(train_data_loader):
                ...
                train_progress_bar.set_postfix(
                    Distance=f"{d.detach().cpu().clone().numpy().mean():.8f}",
                    Loss=f"{running_loss/(step+1):.8f}",
                    Elapsed=f"{(time() - st):.5f}"
                )
...
def eval_model(test_data_loader, global_step, device, model, checkpoint_dir):
    ...
    eval_progress_bar = tqdm(test_data_loader)
    ...
    eval_progress_bar.update(1)
    eval_progress_bar.set_description('Eval Loss: {}'.format(averaged_loss))
