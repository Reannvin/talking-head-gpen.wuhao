...
class AudioVisualDataset(Dataset):
    ...
    def get_segmented_audios(self, audio_embeddings, start_frame):
        ...
        for i in range(start_frame_num, start_frame_num + syncnet_T):
            m = self.crop_audio_window(audio_embeddings, i)
...
class LatentWav2Lip(pl.LightningModule):
    ...
    def __init__(self, hparams):
        ...
        unet_config = UNet2DConditionModel.load_config(f"{self.hparams.unet_ckpt}/config.json")
    ...
    def load_unet(self, unet_ckpt):
        try:
            self.unet.from_pretrained(unet_ckpt, torch_dtype=torch.float16)
    ...
    def forward(self, audio_sequences, face_sequences):
        return self.unet(face_sequences, timestep=self.zero_timestep, encoder_hidden_states=audio_sequences)
    ...
    def get_sync_loss(self, mel, g):
        ...
        if self.hparams.clip_loss:
            ...
            y = torch.ones(g.size(0), 1).half().to(self.device)
    ...
    def reshape_face_sequences(self, tensor):
        ...
        Returns:
            torch.Tensor: A reshaped tensor.
        """
        # 获取输入tensor的维度
        ...
        # 检查channels是否可以被2整除
        if channels % 2 != 0:
            raise ValueError("The channel dimension must be divisible by 2.")
        
        # 将通道维度拆分为两部分
        tensor = tensor.view(batch_size, 2, channels // 2, groups, height, width)
        
        # 纵向拼接两张图片，形状为 [batch_size, channels//2, groups, height*2, width]
        tensor = torch.cat((tensor[:, 0], tensor[:, 1]), dim=3)
        
        # 横向拼接所有组的图像，形状为 [batch_size, channels//2, height*2, width*groups]
        tensor = torch.cat(tuple(tensor[:, :, i] for i in range(groups)), dim=3)
    ...
    def inverse_reshape_face_sequences(self, tensor):
        """
        Inverse operation for the reshape_and_concat function, reconstructing the original tensor
        from a reshaped format of [batch_size, channels//2, height*2, width*groups].
        
        Parameters:
            tensor (torch.Tensor): A tensor with dimensions [batch_size, channels//2, height*2, width*groups].
        
        Returns:
            torch.Tensor: A tensor with dimensions [batch_size, channels, groups, height, width].
        """
        batch_size, half_channels, doubled_height, total_width = tensor.shape
        height = doubled_height // 2  # Calculating original height
        width = height  # Assuming original width equals original height, which was true in the initial tensor

        # Calculating the number of groups from the total width and the individual width
        groups = total_width // width
        
        # 检查输入维度是否合适
        if doubled_height % 2 != 0 or total_width % groups != 0:
            raise ValueError("The dimensions of the input tensor do not match the expected format for splitting.")
        
        # 横向分割
        tensor = tensor.view(batch_size, half_channels, doubled_height, groups, width)
        tensor = tensor.permute(0, 1, 3, 2, 4).contiguous()
        
        # 纵向分割
        tensor = tensor.view(batch_size, half_channels, groups, 2, height, width)
        tensor = torch.cat((tensor[:, :, :, 0], tensor[:, :, :, 1]), dim=1)
        
        # print(f"Inverse reshaped tensor shape: {tensor.shape}")
        return tensor
    ...
    def inverse_reshape_audio_sequences(self, tensor):
        """
        Inverse operation for the reshape_audio_sequences function, reconstructing the original tensor
        from a reshaped format of [batch_size, dim1*1*10, features] to [batch_size, dim1, 1, 10, features].
        
        Parameters:
            tensor (torch.Tensor): A tensor with dimensions [batch_size, dim1*1*10, features].
            
        Returns:
            torch.Tensor: A tensor with dimensions [batch_size, dim1, 1, 10, features].
        """
        dim2 = 1  # Always 1
        dim3 = syncnet_audio_step_size  # Always 10
        batch_size, total_length, features = tensor.shape
        
        # Calculate dim1 based on the total length and known dimensions of dim2 and dim3
        if total_length % (dim2 * dim3) != 0:
            raise ValueError("Total length of features does not match the expected dimensions.")
        dim1 = total_length // (dim2 * dim3)
        
        # Reshape the tensor to its original dimensions
        original_shape_tensor = tensor.view(batch_size, dim1, dim2, dim3, features)
        
        # print(f"Original shape tensor: {original_shape_tensor.shape}")
    ...
    def training_step(self, batch, batch_idx):
        ...
        x = self.reshape_face_sequences(x)
        indiv_audios = self.reshape_audio_sequences(indiv_audios)
        g = self.unet(x, timestep=self.zero_timestep, encoder_hidden_states=indiv_audios).sample
        g = self.inverse_reshape_face_sequences(g)[:, :4, :, :, :]
...
if __name__ == "__main__":
    ...
    parser.add_argument('--unet_ckpt', type=str, help='Path to the unet config file', default='sd-v1-5-unet')
    ...
    if not model.hparams.ckpt:
        model.load_unet(model.hparams.unet_ckpt)

    # Checkpoint callback to save the model periodically
    checkpoint_callback = pl.callbacks.ModelCheckpoint(
        filename='wav2lip-u-{epoch}-{step}-{train_loss:.3f}-{val_loss:.3f}-{val_sync_loss:.3f}',
        save_top_k=3,
        verbose=True,
        monitor='val_loss',
        mode='min'
