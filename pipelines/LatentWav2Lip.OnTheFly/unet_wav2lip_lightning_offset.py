...
class AudioVisualDataset(Dataset):
    def __init__(self, data_root, dataset_name, split, args, audio_root=None, dataset_size=512000):
        ...
        if self.args.offset:
            self.offset_info = parse_info_file_to_hashmap(f'filelists_{dataset_name}/offsets.txt')
...
class LatentWav2Lip(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        if self.hparams.no_image_loss:
            ...
            if self.hparams.triplet_loss:
                ref_image = x[:, 3:, :, :]
                d_neg = self.get_lpips_loss(g_image, ref_image)
                d_pos = lpips_loss
                d_gt_and_ref = self.get_lpips_loss(gt, ref_image)
                
                triplet_loss = torch.relu(d_pos - d_neg / (d_gt_and_ref * 2) + 1)
                image_loss += self.hparams.triplet_loss_weight * triplet_loss
                
                self.log('train_triplet_loss', triplet_loss, prog_bar=True, sync_dist=True, on_epoch=True)
                self.log('train_d_pos', d_pos, prog_bar=True, sync_dist=True, on_epoch=True)
                self.log('train_d_neg', d_neg, prog_bar=True, sync_dist=True, on_epoch=True)
                self.log('train_d_gt_and_ref', d_gt_and_ref, prog_bar=True, sync_dist=True, on_epoch=True)
        ...
        self.log('train_image_loss', image_loss, prog_bar=True, sync_dist=True, on_epoch=True)
    ...
    def validation_step(self, batch, batch_idx):
        ...
        g_latent, g_image, latent_x, g_base_image = model_out.g_latent, model_out.g_image, model_out.latent_face_sequences, model_out.g_base_image     
        ...
        if self.hparams.no_image_loss:
            ...
            if self.hparams.triplet_loss:
                ...
                self.log('val_triplet_loss', triplet_loss, prog_bar=True, sync_dist=True, on_epoch=True)
                self.log('val_d_pos', d_pos, prog_bar=True, sync_dist=True, on_epoch=True)
                self.log('val_d_neg', d_neg, prog_bar=True, sync_dist=True, on_epoch=True)
                self.log('val_d_gt_and_ref', d_gt_and_ref, prog_bar=True, sync_dist=True, on_epoch=True)
        ...
        self.log('val_image_loss', image_loss, prog_bar=True, sync_dist=True, on_epoch=True)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--triplet_loss', action='store_true', help='Use triplet loss for training')
