...
def find_subdirs(root_dir):
    ...
    for root, dirs, _ in os.walk(root_dir, followlinks=True):
        ...
        if normalized_root.count(os.sep) - root_dir.count(os.sep) == 1:
            # Extract the last two parts of the path
            path_parts = normalized_root.split(os.sep)[-1:]
            simplified_path = os.sep.join(path_parts)
            if 'whisper.npy.5' in os.listdir(os.path.join(root_dir,simplified_path)):
