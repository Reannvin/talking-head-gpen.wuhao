...
from stylegan3.discrimintor import Discriminator
...
class LatentWav2LipOutput:
    def __init__(self, g_latent, g_image=None, g_base_image=None):
...
class LatentWav2Lip(pl.LightningModule):
    def __init__(self, hparams):
        ...
        stylegan_enabled = args.enable_stylegan_loss
        if stylegan_enabled:
    ...
    def get_discriminator_loss(self, gt, gen):
        gt = gt[:, :, 0, :, :]
        gen = gen[:, :, 0, :, :]

        gt = torch.nn.functional.interpolate(gt, size=(512, 512), mode='bilinear', align_corners=False)
        gen = torch.nn.functional.interpolate(gen, size=(512, 512), mode='bilinear', align_corners=False)
    
        d_loss, real_logits, fake_logits = self.discriminator.run_discriminator(gt, gen)
        return -fake_logits.mean(), d_loss
    ...
    def forward(self, audio_sequences, face_sequences, with_image=True, with_base_image=False):
        ...
        return LatentWav2LipOutput(g_latent, g_image, g_base_image)
    ...
    def training_step(self, batch, batch_idx):
        ...
        # Dropout operations (reference, image, audio)
        if self.hparams.dropout_ref and self.hparams.drop_ref_prob > 0. and random.random() < self.hparams.drop_ref_prob:
            masked, ref = x.split([3, 3], dim=1)
            x = torch.cat([masked, torch.zeros_like(ref)], dim=1)
        
        if self.hparams.image_drop_prob > 0. and random.random() < self.hparams.image_drop_prob:
            x = torch.zeros_like(x)
        
        if self.hparams.audio_drop_prob > 0. and random.random() < self.hparams.audio_drop_prob:
            indiv_audios = torch.zeros_like(indiv_audios)
        
        model_out = self(indiv_audios, x, not self.hparams.no_image_loss)
        g_latent, g_image = model_out.g_latent, model_out.g_image
        
        if self.hparams.no_image_loss:
            image_recon_loss = lpips_loss = 0.
        else:
            if self.hparams.no_full_image_loss:
                half_g_image = g_image[:, :, :, g_image.size(3) // 2:, :]
                half_gt = gt[:, :, :, gt.size(3) // 2:, :]
                image_recon_loss = self.recon_loss(half_g_image, half_gt)
                lpips_loss = self.get_lpips_loss(half_g_image, half_gt)
            else:
        ...
        sync_loss = self.get_sync_loss(audio_cropped, g_image[:, :, :, g_image.size(3) // 2:, :]) if self.hparams.syncnet and self.hparams.syncnet_wt > 0. else 0.
        ...
        if stylegan_enabled:
            stylegan_loss, d_loss = self.get_discriminator_loss(gt, g_image)
            recon_loss += 0.1 * stylegan_loss
            # loss += 0.1 * stylegan_loss
            loss = self.hparams.syncnet_wt * sync_loss + (1 - self.hparams.syncnet_wt) * recon_loss
        ...
        if stylegan_enabled:
            self.log('train_stylegan_loss', stylegan_loss, prog_bar=True, sync_dist=True, on_epoch=True)
            self.log('d_loss', d_loss, prog_bar=True, sync_dist=True, on_epoch=True)
    ...
    def validation_step(self, batch, batch_idx):
        ...
        recon_loss = latent_recon_loss + self.hparams.image_loss_wt * image_loss + 0.1 * stylegan_loss
    ...
    def save_sample_images(self, x, g_base, g, gt, global_step, checkpoint_dir):
        refs = x[:, 3:, :, :, :]
        inps = x[:, :3, :, :, :]
    ...
    def configure_optimizers(self):
        ...
        if args.enable_stylegan_loss:
            g_optimizer = torch.optim.AdamW([p for p in self.unet.parameters() if p.requires_grad], lr=1e-4)
            d_optimizer = torch.optim.AdamW([p for p in self.discriminator.get_parameters() if p.requires_grad], lr=1e-4)
            
            # 使用 LambdaLR 包装自定义的调度器
            g_lr_scheduler = torch.optim.lr_scheduler.LambdaLR(g_optimizer, lr_lambda=scheduler.schedule)
            d_lr_scheduler = torch.optim.lr_scheduler.LambdaLR(d_optimizer, lr_lambda=scheduler.schedule)
            
            return [{"optimizer": g_optimizer, "lr_scheduler": {"scheduler": g_lr_scheduler, "interval": "step", "frequency": 1}},
                    {"optimizer": d_optimizer, "lr_scheduler": {"scheduler": d_lr_scheduler, "interval": "step", "frequency": 1}}]
        else:
...
if __name__ == "__main__":
    ...
    parser.add_argument('--enable_stylegan_loss', action='store_true', help="Enable Discriminator de StyleGAN as Loss function")
    ...
    # 设置日志目录和实验名称
    if args.wandb:
        logger = WandbLogger(project='styleGAN_wav2lip', entity=args.wandb_entity)
    else:
    ...
    if args.enable_stylegan_loss: 
        trainer = Trainer(**common_trainer_args) 
    else:
        trainer = Trainer(
            **common_trainer_args,
