...
def split_whisper_files(data_root, syncnet_T):
    ...
    for video_path, person_id, video_id in videos_list:
        whisper_path = os.path.join(video_path, f'whisper.npy.{syncnet_T}')
        if os.path.exists(whisper_path):
            whisper_list = torch.load(whisper_path)
            for i, array in enumerate(whisper_list):
                save_path = os.path.join(video_path, f'{i}.npy.{syncnet_T}')
                np.save(save_path, array)
            print(f'Split whisper.npy.{syncnet_T} in {video_path} into {len(whisper_list)} files.')
        else:
            print(f'whisper.npy.{syncnet_T} not found in {video_path}')
