...
def get_landmark_and_bbox(model, fa, video_path, upperbondrange=0):
    frames = read_img_from_video(video_path)
...
def get_landmark(model,  video_path):
    frames = read_img_from_video(video_path)
    landmarks = []
    for fb in frames:
        results = inference_topdown(model, np.asarray(fb))
        results = merge_data_samples(results)
        keypoints = results.pred_instances.keypoints
        face_land_mark = keypoints[0][48:68]
        face_land_mark = face_land_mark.astype(np.int32)
...
def get_arc_face(model, video_path,image_size=256):
    frames = read_img_from_video(video_path)
...
def process_HDTF_landmark_arcface(rank, world_size, data_root, save_dir):
    # dist.init_process_group("nccl", rank=rank, world_size=world_size)
    setup(rank, world_size)
    torch.cuda.set_device(rank)
    ...
    for video in pbar:
        ...
        try:
          ...
          #  print(f"Processing video: {video}")
            ...
            metric_list,landmark_list,arc_faces = get_arc_face(model, video_path)
...
def main():
    ...
    parser.add_argument('--world_size', type=int, help='Number of GPUs to use')
