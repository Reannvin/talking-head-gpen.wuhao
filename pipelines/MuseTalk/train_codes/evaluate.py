...
#         set_seed(args.seed)
    ...
    unet = UNet(unet_config="../models/musetalk/musetalk.json", model_path =args.unet_ckpt)
    unet = unet.model
    print(unet)  
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        unet.train()
        loss_list, loss_lip_list, loss_latents_list = [], [], []
        ...
        for step, (ref_image, image, masked_image, masks, audio_feature) in enumerate(train_data_loader):
            ...
            # Predict the noise residual
                ...
                if args.reconstruction: # decode the image from the predicted latents
                    ...
                    loss_lip_list.append(loss_lip.detach().item())
                    loss_latents_list.append(loss_latents.detach().item())
...
#                   
            ...
            logs = {"loss_lip": loss_lip.detach().item(),"loss_latents": loss_latents.detach().item(),"loss": loss.detach().item(), 
...
#             accelerator.log(logs, step=global_step)
            ...
            accelerator.log(
                {
                    ...
                    "loss/lip_loss": logs["loss_lip"],
                    "loss/latents_loss": logs["loss_latents"],
        ...
        if args.reconstruction:
            # accelerator.log(
            #     {
            #         "loss/epoch_loss": np.mean(loss_list),
            #         "loss/epoch_lip_loss": np.mean(loss_lip_list),
            #         "loss/epoch_latents_loss": np.mean(loss_latents_list),
            #     },
            #     step=global_step,
            # )
            print(f"epoch:{epoch} loss:{np.mean(loss_list)} lip_loss:{np.mean(loss_lip_list)} latents_loss:{np.mean(loss_latents_list)}")
